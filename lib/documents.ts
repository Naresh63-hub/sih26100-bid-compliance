...
export async function readDocument(file:File, role:'tender'|'bid'):Promise<EvidenceDocument> {
 ...
 let pages:string[]=[];
 if(/\.txt$/i.test(file.name)) pages=new TextDecoder().decode(bytes).split('\f');
 else {
 ...
 if(!pages.some(p=>p.trim())) throw new Error('No readable text was found. Scanned PDFs need OCR before import; OCR is not included.');
 if(pages.join('').length>1_000_000) throw new Error('Extracted text exceeds the prototype limit of 1 million characters.');
...
export function extractRequirements(doc:EvidenceDocument):Requirement[] {
 ...
 doc.pages.forEach((page,index)=>page.split('\n').forEach(raw=>{
  const line=raw.trim(); if(!line) return;
  const numeric=patterns.find(p=>line.toLowerCase().includes(p.key));
  const portal=line.match(/\b(GST|PAN|Udyam|EPFO|ESIC|DigiLocker|NSIC|MCA21)\b/i);
  if(!numeric&&!portal&&!/\b(shall|must|minimum|required)\b/i.test(line)) return;
  let minimum:number|undefined;
  if(numeric) {
   const match=line.match(new RegExp('^minimum '+numeric.key+'\\s*[:=]\\s*(\\d+(?:\\.\\d+)?)\\s*'+numeric.unit+'\\s*$','i'));
   if(match) minimum=Number(match[1]);
  }
  result.push({id:`REQ-${String(result.length+1).padStart(3,'0')}`,title:line.slice(0,240),category:numeric?.category||(portal?'Statutory':'Other'),key:numeric?.key||portal?.[0]||line.slice(0,60),unit:numeric?.unit||'',kind:minimum!==undefined?'minimum':portal?'portal':'manual',minimum,source:doc.name,page:index+1,weight:10});
 }));
 return result;
}
